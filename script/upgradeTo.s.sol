...
contract UpgradeTo is Script {
    ...
    function run() external {
        ...
        ProxyAdmin admin = ProxyAdmin(address(uint160(uint256(vm.load(_veggiaProxy, ADMIN_SLOT)))));

        VeggiaERC721 newImplementation = new VeggiaERC721();

        admin.upgradeAndCall(ITransparentUpgradeableProxy(_veggiaProxy), address(newImplementation), abi.encode());

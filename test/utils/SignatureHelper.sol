...
library SignatureHelper {
    ...
    bytes32 private constant WITHDRAWREQUEST_TYPEHASH = keccak256("WithdrawRequest(address to,uint256 amount)");
    ...
    function signBtcRewardAs(VeggiaBtcPool veggiaBtcPool, bytes32 signer, address to, uint256 amount)
        public
        view
        returns (bytes memory signature)
    {
        bytes32 DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
                keccak256(bytes("VeggiaBtcPool")),
                keccak256(bytes("1")),
                block.chainid,
                address(veggiaBtcPool)
            )
        );

        bytes32 structHash = keccak256(abi.encode(WITHDRAWREQUEST_TYPEHASH, to, amount));

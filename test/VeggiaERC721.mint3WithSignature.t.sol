...
contract VeggiaERC721MintWithSignatureTest is Test, ERC721Holder {
	...
	function test_mint3WithSignature(
		...
		veggia = DeployHelper.deployVeggia(
			address(this),
			address(this),
			serverSigner,
			"http://localhost:4000/"
		...
		VeggiaERC721.MintRequest memory req = VeggiaERC721.MintRequest(
			user,
			index,
			isPremium
		);
		bytes memory signature = SignatureHelper.signMint3As(
			veggia,
			bytes32(signer),
			user,
			isPremium,
	...
	function test_MintWithSignatureWrongSigner(
		string memory random,
		uint256 index,
		bool isPremium,
		address user
		...
		vm.expectRevert(
			abi.encodeWithSelector(VeggiaERC721.INVALID_SIGNATURE.selector)
		);
		vm.prank(user);
		veggia.mint3WithSignature(req, signature);
	...
	function test_MintWithSignatureReusedSignature(
		string memory random,
		uint256 index,
		bool isPremium,
		address user
		...
		// Reuse signature
		vm.expectRevert(
			abi.encodeWithSelector(VeggiaERC721.SIGNATURE_REUSED.selector)
		);
		vm.prank(user);
		veggia.mint3WithSignature(req, signature);
	}

	function test_MintWithSignatureInvalidSender(
		string memory random,
		uint256 index,
		bool isPremium,
		address user
		...
		vm.expectRevert(
			abi.encodeWithSelector(
				VeggiaERC721.INVALID_SENDER.selector,
				invalidSender,
				user
			)
		);
